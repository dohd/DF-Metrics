...
<html>
    ...
    <body>
        ...
        <table width="100%" style="font-size:10pt;margin-top:10px;">
            ...
            <tr>
                <td style="text-align: center;" width="100%" class="headerData">
                    <span style="font-size:16pt; color:#0f4d9b; text-transform:uppercase;"><b>{{ $meta['programme']['name'] }}</b></span>
                </td>
            </tr>
        </table>
        ...
        <table class="items items-table" cellpadding=8 width="100%">
            <thead>
                <tr class="heading">
                    <th>#</th>
                    <th>Team</th> 
                    @foreach ($meta['age_groups'] as $row)
                        <th>{{ $row->bracket }}</th>
                    @endforeach                   
                </tr>
            </thead>
            <tbody>
                @foreach ($records as $i => $team)
                    <tr>
                        ...
                        <td>{{ $team->name }}</td>
                        @foreach ($meta['age_groups'] as $row)
                            <td>{{ $team->age_groups->get($row->id) }}</td>
                        @endforeach   
                    </tr>
                @endforeach
            </tbody>
