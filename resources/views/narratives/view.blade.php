@extends('layouts.core')
@section('title', 'Narratives')
    
@section('content')
    @include('narratives.header')
    <div class="card">
        <div class="card-body">
            <h6 class="card-title">Narrative Report</h6>
            <div class="card-content p-2">
                <p>Date: <b>{{ dateFormat($narrative->date, 'd-M-Y') }}</b></p>
                <h3 class="text-center text-primary mt-2 mb-2 fw-bold">{{ $narrative->title }}</h3>
                <div class="mb-3">
                    <h5><u>Introduction</u></h5>
                    <div>{!! @$narrative->introduction !!}</div>
                </div>
                <div class="mb-3">
                    <h5><u>Objectives</u></h5>
                    <div>{!! @$narrative->objectives !!}</div>
                </div>
                <div class="mb-3">
                    <h5><u>Activities</u></h5>
                    <div>{!! @$narrative->activities !!}</div>
                </div>
                <div class="mb-3">
                    <h5><u>Challenges</u></h5>
                    <div>{!! @$narrative->challenges !!}</div>
                </div>
                <div class="mb-3">
                    <h5><u>Outcomes</u></h5>
                    <div>{!! @$narrative->outcomes !!}</div>
                </div>
                <div class="mb-3">
                    <h5><u>Reflections</u></h5>
                    <div>{!! @$narrative->reflections !!}</div>
                </div>
                <div class="mb-3">
                    <h5><u>Conclusion</u></h5>
                    <div>{!! @$narrative->conclusion !!}</div>
                </div>
                <div class="row">
                    @php
                        $images = [$narrative->image1, $narrative->image2, $narrative->image3];
                    @endphp
                    @if (array_filter($images))
                        <h5><u>Images</u></h5>
                    @endif
                    
                    @if (isset($images[0]))
                        <div class="col-md-4">
                            <div style="width: 300px; height: 250px;">
                                <img src="{{ route('storage.file_render', 'images,narratives,' . $images[0]) }}" alt="image1" style="width: 100%; height: 100%; border-radius: 8px;">
                            </div>
                            <div>
                                <span class="del" style="cursor: pointer;" name="image1">
                                    <i class="bi bi-trash text-danger icon-xs"></i>
                                </span>
                                {{ $narrative->caption1 }}
                            </div>
                        </div>
                    @endif
                    @if (isset($images[1]))
                        <div class="col-md-4">
                            <div style="width: 300px; height: 250px;">
                                <img src="{{ route('storage.file_render', 'images,narratives,' . $images[1]) }}" alt="image1" style="width: 100%; height: 100%; border-radius: 8px;">
                            </div>
                            <div>
                                <span class="del" style="cursor: pointer;" name="image2">
                                    <i class="bi bi-trash text-danger icon-xs"></i>
                                </span>
                                {{ $narrative->caption2 }}
                            </div>
                        </div>
                    @endif
                    @if (isset($images[2]))
                        <div class="col-md-4">
                            <div style="width: 300px; height: 250px;">
                                <img src="{{ route('storage.file_render', 'images,narratives,' . $images[2]) }}" alt="image1" style="width: 100%; height: 100%; border-radius: 8px;">
                            </div>
                            <div>
                                <span class="del" style="cursor: pointer;" name="image3">
                                    <i class="bi bi-trash text-danger icon-xs"></i>
                                </span>
                                {{ $narrative->caption3 }}
                            </div>
                        </div>
                    @endif
                </div>
            </div>
        </div>
    </div>
@stop

@section('script')
<script>
    $(document).on('click', '.del', function() {
        const field = $(this).attr('name');
        const narrative_id = @json($narrative->id);
        const url = @json(route('narratives.delete_image'));
        if (confirm('Are you sure?')) {
            $.post(url, {narrative_id, field})
            .done((data) => flashMessage(data))
            .catch((data) => flashMessage(data));
        }
    });
</script>
@endsection

