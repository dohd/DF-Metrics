...
<div class="modal fade" id="status_modal" tabindex="-1" aria-labelledby="status_modal_label" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            {{ Form::model($study_material, ['route' => array('study_materials.update', $study_material), 'method' => 'PATCH']) }}
                        ...
                        <div class="col-12">
                            ...
                            <select name="status" id="status" class="form-select">
                                @foreach (['pending', 'approved', 'review'] as $item)
                                    <option value="{{ $item }}" {{ $study_material->status == $item? 'selected' : '' }}>{{ ucfirst($item) }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
